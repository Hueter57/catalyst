...
func (_q *ReminderQuery) QueryTask() *TaskQuery {
...
func (_q *ReminderQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *ReminderQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (_q *ReminderQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *ReminderQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (_q *ReminderQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (_q *ReminderQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *ReminderQuery) WithTask(opts ...func(*TaskQuery)) *ReminderQuery {
	query := (&TaskClient{config: _q.config}).Query()
...
//		GroupBy(reminder.FieldRemindAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *ReminderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Reminder, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, reminder.ForeignKeys...)
	}
	...
	if query := _q.withTask; query != nil {
		if err := _q.loadTask(ctx, query, nodes, nil,
			func(n *Reminder, e *Task) { n.Edges.Task = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ReminderQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*Reminder, init func(*Reminder), assign func(*Reminder, *Task)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Reminder)
	for i := range nodes {
		if nodes[i].task_reminder == nil {
			continue
		}
		fk := *nodes[i].task_reminder
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "task_reminder" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
