...
func (_q *TaskQuery) QueryCategory() *CategoryQuery {
...
func (_q *TaskQuery) QueryAssignee() *AssigneeQuery {
	query := (&AssigneeClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TaskQuery) QueryReminder() *ReminderQuery {
...
func (_q *TaskQuery) WithCategory(opts ...func(*CategoryQuery)) *TaskQuery {
	query := (&CategoryClient{config: _q.config}).Query()
...
func (_q *TaskQuery) WithAssignee(opts ...func(*AssigneeQuery)) *TaskQuery {
	query := (&AssigneeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) WithReminder(opts ...func(*ReminderQuery)) *TaskQuery {
	query := (&ReminderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := _q.withCategory; query != nil {
		if err := _q.loadCategory(ctx, query, nodes,
			func(n *Task) { n.Edges.Category = []*Category{} },
			func(n *Task, e *Category) { n.Edges.Category = append(n.Edges.Category, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAssignee; query != nil {
		if err := _q.loadAssignee(ctx, query, nodes, nil,
			func(n *Task, e *Assignee) { n.Edges.Assignee = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withReminder; query != nil {
		if err := _q.loadReminder(ctx, query, nodes, nil,
			func(n *Task, e *Reminder) { n.Edges.Reminder = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TaskQuery) loadCategory(ctx context.Context, query *CategoryQuery, nodes []*Task, init func(*Task), assign func(*Task, *Category)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(task.CategoryTable)
		s.Join(joinT).On(s.C(category.FieldID), joinT.C(task.CategoryPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(task.CategoryPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(task.CategoryPrimaryKey[0]))
...
func (_q *TaskQuery) loadAssignee(ctx context.Context, query *AssigneeQuery, nodes []*Task, init func(*Task), assign func(*Task, *Assignee)) error {
	...
	query.withFKs = true
	query.Where(predicate.Assignee(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.AssigneeColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.task_assignee
		if fk == nil {
			return fmt.Errorf(`foreign-key "task_assignee" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "task_assignee" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *TaskQuery) loadReminder(ctx context.Context, query *ReminderQuery, nodes []*Task, init func(*Task), assign func(*Task, *Reminder)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Task)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Reminder(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.ReminderColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.task_reminder
		if fk == nil {
			return fmt.Errorf(`foreign-key "task_reminder" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "task_reminder" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
