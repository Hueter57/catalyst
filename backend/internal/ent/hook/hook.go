...
// The AssigneeFunc type is an adapter to allow the use of ordinary
// function as Assignee mutator.
type AssigneeFunc func(context.Context, *ent.AssigneeMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AssigneeFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.AssigneeMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AssigneeMutation", m)
}
